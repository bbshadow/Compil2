...
n_exp *expression(){
  ...
  if (est_premier(uniteCourante,_conjonction_)||est_premier(EPSILON,_conjonction_))
  {
    ...
    herite_fils = $2;
    affiche_balise_fermante(__FUNCTION__, 1);
    return expressionBis(herite_fils);
  }

  else {
    ( "Erreur de syntaxe 37 : %d", uniteCourante );
    exit( -1 );
  }
}
...
n_exp *expressionBis( n_exp *herite ){
  ...
  else if (uniteCourante == INTERROGATION)
  {
    ...
    if (uniteCourante == DEUXPOINTS)
    {
      ...
      $2 = expression(); 
      affiche_balise_fermante(__FUNCTION__, 1);
      return cree_n_exp_qqchosequonapas(herite, $1, $2);//TODO
    } else {
...
n_exp *expArithBis(  n_exp *herite  ){
  ...
  if (uniteCourante == PLUS)
  {
   ...
   affiche_balise_fermante(__FUNCTION__, 1);
   return expArithBis(herite_fils);
  } 
...
n_exp *termeBis( n_exp *herite ) {
    ...
    if(uc == FOIS){
      ...
      return termeBis( herite_fils);
    }

    if(est_suivant(uc, _termeBis_)){
